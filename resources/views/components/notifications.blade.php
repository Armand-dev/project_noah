<div class="w-full h-full top-0 overflow-y-auto overflow-x-hidden fixed sticky-0 shadow-md z-50" id="chec-div">
    <div class="w-full absolute z-10 right-0 h-full overflow-x-hidden transform translate-x-0 transition ease-in-out duration-100" id="notification">
        <div id="actual-notification-drawer" class="2xl:w-4/12 bg-gray-50 h-screen overflow-y-auto p-8 absolute right-0">
...
<script type="module">
    let notificationDrawer = document.getElementById("notifications-go-here");

    const addNotification = (notification) => {
        switch (notification.type) {
            case 'App\\Notifications\\ExportCompleted':
                let notificationContent = `
                        <div class="w-full p-3 mt-4 bg-white rounded flex">
                            ...
                            <div class="pl-3">
                                <p tabindex="0" class="focus:outline-none text-sm leading-none">` + notification.title + `<a href="` + notification.url + `" class="text-indigo-700"> Download</a></p>
                                <p tabindex="0" class="focus:outline-none text-xs leading-3 pt-1 text-gray-500">` + notification.created_at ?? 'just now' + `</p>
                            </div>
                        </div>`;

                notificationDrawer.insertAdjacentHTML('afterbegin', notificationContent);
                break;
        }
    }
    ...
    //         </div>
    //         <div class="ms-3 text-sm font-normal">`+ notification.title +`</div>
    ...
    //     `;
    //
    //     document.body.insertAdjacentHTML('beforeend', toastContent);
    // }
