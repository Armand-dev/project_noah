<x-app-layout>
    ...
    <div class="p-1 overflow-hidden bg-white rounded-md shadow-md dark:bg-dark-eval-1">
        <div class="container mx-auto" dt-scope="chat">
            <div class="min-w-full border rounded lg:grid lg:grid-cols-3">
                <div class="hidden border-r border-gray-300 lg:block lg:col-span-1 ">
                    <div class="mx-3 my-3">
                        ...
                        <li>
                            <a
                                ...
                                <img class="object-cover w-10 h-10 rounded-full"
                                     src="https://ui-avatars.com/api/?name={{ str_replace(" ", "+", $company->name) }}&background=random" alt="username" />
                                <div class="w-full pb-2">
                                    <div class="flex justify-between">
                                        <span class="block ml-2 font-semibold text-gray-600">{{ $company->name }}</span>
{{--                                        <span class="block ml-2 text-sm text-gray-600">25 minutes</span>--}}
                                    </div>
                                    <span class="block ml-2 text-sm text-gray-600">Company chat</span>
                                </div>
                            </a>
                        </li>
                    </ul>
                ...
                <div class="lg:col-span-2 lg:block">
                    <div class="w-full">
                        <div class="relative flex items-center p-3 border-b border-gray-300">
                            <img class="object-cover w-10 h-10 rounded-full"
                                 src="https://ui-avatars.com/api/?name={{ str_replace(" ", "+", $company->name) }}&background=random" alt="username" />
                            <span class="block ml-2 font-bold text-gray-600">{{ $company->name }}</span>
                            <span class="absolute w-3 h-3 bg-green-600 rounded-full left-10 top-3">
                            </span>
                        </div>
                        <div class="relative w-full p-6 overflow-y-auto h-[40rem]">
                            <ul class="space-y-2" dt-scope="chatbox">
                                {{-- ## MESSAGES GO HERE--}}
                            </ul>
                        </div>
...
{{--                            </button>--}}
    ...
    <script type="module">
        let chat = document.querySelector('[dt-scope="chatbox"]');
        ...
        Echo.private(`companies.{{ $company->uuid }}`)
            .listen('SendMessage', (e) => {
                appendMessage(e);
            });

        document.querySelector('[dt-scope="chat"]').addEventListener('keydown', (e) => {
            if(e.key === 'Enter') {
                sendMessage();
            }
        });
        ...
        let appendMessage = (messageData) => {
            let message = '';
            messageData.meta.is_myself = messageData.meta.sent_by.id === {{ auth()->user()->id }};
            ...
            chat.querySelectorAll('[dt-scope="message"]')[chat.querySelectorAll('[dt-scope="message"]').length - 1].scrollIntoView();
        }
    </script>
</x-app-layout>
